  ...
  //-----------------------------------------------
	if((g_GINavInfo.SYSFlag<3))
	{
	 ...
	 {	
		...
		//经纬度赋值
		//------------------------------------------------------		
		g_GINavInfo.InsAlignKind=1;
		POS_First_Flag=1;
			
	  g_GINavInfo.Position.Lat = Lat_Back;
		g_GINavInfo.Position.Lon = Lon_Back;
		g_GINavInfo.Position.Alt = 0;
		...
		g_GINavInfo.INSState |= INS_POSVEL_GOOD;  //
    ...
    //------------------------------------------------------
		//姿态赋值
		//------------------------------------------------------	
    g_GINavInfo.Euler.Theta = 0;//Pitch_Back*DEG2RAD;
    g_GINavInfo.Euler.Gamma = 0;//Roll_Back*DEG2RAD;
		g_GINavInfo.Euler.Phi   = Head_Back*DEG2RAD;		
		
		Euler2Quat(&g_GINavInfo.Euler, &g_GINavInfo.Quat_bn);
		Quat2CM(&g_GINavInfo.Quat_bn, &g_GINavInfo.CM_bn);

		g_GINavInfo.dVelocity.Ve = g_GINavInfo.dVelocity.Vn = g_GINavInfo.dVelocity.Vu = 0.0;      //gty ??????,??....
        ...
        g_GINavInfo.INSState |= INS_LEVELATT_GOOD;
		g_GINavInfo.INSState |= INS_HEADING_INIT; //
		...
		for(i=0;i<3;i++)
		{
		 g_GINavInfo.ImuCfg.AccBias[i]=0.0;
		}
		//------------------------------------------------------
	...
	{
		g_GINavInfo.InsAlignKind=2;
		POS_First_Flag=0;
		
		if (g_GINavInfo.ResetAlignFlag > 0)             //如果复位则....等待
		{
			if ((g_GINavInfo.GstStatus >= 1) && (g_GINavInfo.GST_Diff<g_GINavInfo.GstDetaMin))//隧道里面复位之后，要等待好的位置信息...
				P_RunFlag = 1;
			else
				P_RunFlag = 0;
		}
	 ...
	 if (pGnssData && IS_POS_VALID(pGnssData->NavFlag) && (pGnssData->SatUseNum>3) && P_RunFlag)   //如果GPS经纬高度三维定位都可以，则....
	 { 
		...
		if (IS_LEVEL_VEL_VALID(pGnssData->NavFlag)&&RunFlag)      //如果GPS水平速度和方向都可以，则.....
		{
	       ...
	       HeadingTemp = atan2(g_GINavInfo.Velocity.Ve, g_GINavInfo.Velocity.Vn);//gty 把航向角复制给INS
				...
				if((INS_DGet_Flag!=0)&&(g_GINavInfo.Head_Flag))    //如果打开
				{
