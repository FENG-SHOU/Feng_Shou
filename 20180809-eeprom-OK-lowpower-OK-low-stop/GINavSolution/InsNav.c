...
void HandleCompensate(PIMU_DATA_T pImuData)
{
	S32 i, j;
	...
	if (g_GINavInfo.Ini3_Flag == 1)
	{
		for (i = 0; i < INS_UPDATE_SAMPLE_NUM; i++)
		{
			...
			for (j = 0; j < 3; j++)
			{
				g_GINavInfo.AccD[j]   = temp2Acc[j];
				g_GINavInfo.GyrD[j]   = temp2Gyro[j];
				//pImuData->Acc[i][j] = tempAcc[j];
				//pImuData->Gyro[i][j] = tempGyro[j];
			}
		}
	}
...
void IMUCompensate(PIMU_DATA_T pImuData)  //gty IMU安装角矩阵补偿，并且完成偏置补偿....
{
	...
	for (i = 0; i < INS_UPDATE_SAMPLE_NUM; i++)
	{
		...
		for (j = 0; j < 3; j++) //Compensate Bias
		{
			...
			{
				pImuData->Gyro[i][j] = temp1Gyro[j] - (g_GINavInfo.ImuCfg.GyroBias[j] + g_GINavInfo.GyrDrift[j] * DEG2RAD) * dt;     //gty 在此处已经进行过陀螺仪偏置的补偿处理
...
void InitInstallMat(PIMU_DATA_T pImuData)
{
  ...
  if((g_GINavInfo.SYSFlag<3))
	{
	 if(RSE_DGet_Flag!=1)                         //不是车库启动，则静态，否则，直接处理，
	 {	
	  if (g_GINavInfo.StaticCount == 0)           //gty 如果动态，则返回.....
		 return;
   }
...
void INSUpdate(PIMU_DATA_T pImuData, BOOL bPosUpdate, BOOL bVelUpdate, BOOL bAttUpdate)	//n:NED Frame, b:FRD Frame
{
	S32 i;
	FLOAT64 dt = pImuData->MsrInterval / 1000.0;
	FLOAT64 MidV[3], MidWen[3], temp1[3], temp2[3];
